...
ggplot(water_missing, aes(x = variable, y = percent_missing)) +
  geom_col()
  # Add a label indicating higher is worse.
```
...
With a ROC AUC of 0.66, the xgboost outperforms the decision tree, but not by much.
...
ggplot(comparison, aes(x = model, y = roc_auc)) +
  ...
  labs(y = "ROC AUC")
  # TODO indicate that higher is better
```
